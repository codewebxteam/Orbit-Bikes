...
export default function Footer() {
  ...
  return (
    <footer id="contact" className="section-shell border-t border-white/10 bg-[#07100c] pb-10">
      <div className="content-wrap">
        <div className="grid gap-6 lg:grid-cols-[1.1fr_0.8fr_1.1fr]">
          <div className="panel-surface flex flex-col justify-between p-6 sm:p-8">
            <div>
              <p className="text-[10px] uppercase tracking-[0.4em] text-[var(--color-brand)] sm:text-xs">Orbit electric mobility</p>
          ...
          <div className="panel-surface p-6 sm:p-8">
            <p className="text-[10px] uppercase tracking-[0.3em] text-slate-500 sm:text-xs">Get in touch</p>
            <div className="mt-6 grid gap-5 text-sm">
              <div className="flex items-center gap-4 group">
                ...
                <a href="tel:9458502838" className="text-slate-300 transition-colors hover:text-white">+91 94585 02838</a>
              </div>
              <div className="flex items-center gap-4 group">
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white/5 text-[var(--color-brand)] transition-colors group-hover:bg-[rgba(110,220,140,0.15)]">
                  <FiMail size={16} />
                </div>
                <a href="mailto:support@orbitebikes.in" className="text-slate-300 transition-colors hover:text-white">support@orbitebikes.in</a>
              </div>
              <div className="flex items-start gap-4 group">
                ...
                <div>
                  ...
                  <p className="mt-1 text-[11px] leading-relaxed text-slate-400 line-clamp-2">
                    G-1/700–701, Road No. 22, Phase II, RIICO Industrial Area, Bhiwadi, Rajasthan – 301001
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="mt-8 rounded-3xl border border-white/5 bg-white/[0.03] p-5">
              <p className="text-sm font-bold text-white">Subscribe to updates</p>
              <form onSubmit={handleSubscribe} className="mt-4 flex gap-2">
                ...
                />
                <button type="submit" disabled={status === 'sending'} className="orbit-button-primary h-[44px] px-6 font-bold uppercase tracking-wider text-[11px]">
                  {status === 'sending' ? '...' : 'Join'}
                </button>
              </form>
              {status === 'done' && <p className="mt-3 text-[11px] text-emerald-400 flex items-center gap-2"><div className="h-1 w-1 rounded-full bg-current" /> Success!</p>}
            </div>
          </div>
        </div>
        ...
        <div className="mt-12 flex flex-col items-center justify-between gap-6 border-t border-white/5 pt-10 sm:flex-row">
          ...
          <div className="flex items-center gap-8">
            ...
            <p className="text-[11px] font-medium uppercase tracking-[0.2em] text-slate-500 hidden sm:block">
              Made With ❤️ by <a href="https://codewebx.in" target="_blank" rel="noreferrer">CodeWebX</a>
            </p>
          </div>
        </div>
      </div>
